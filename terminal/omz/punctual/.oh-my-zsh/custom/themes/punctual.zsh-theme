...
#  This file includes some minor modifications, based on the original file
#  (https://raw.githubusercontent.com/dannynimmo/punctual-zsh-theme/v0.4.0/punctual.zsh-theme)
#
#  by Martin (https://github.com/martinjnilsen)
...
PUNCTUAL_SHOW_INDENT="${PUNCTUAL_SHOW_INDENT:-true}";
...
PUNCTUAL_SHOW_PYTHON_ENVIRONMENT="${PUNCTUAL_SHOW_PYTHON_ENVIRONMENT:-false}";
...
PUNCTUAL_PYTHON_ENVIRONMENT_COLOUR="${PUNCTUAL_PYTHON_ENVIRONMENT_COLOUR:-yellow}";
...
PUNCTUAL_PYTHON_ENVIRONMENT_BOLD="${PUNCTUAL_PYTHON_ENVIRONMENT_BOLD:-false}";

PUNCTUAL_TIMESTAMP_FORMAT="${PUNCTUAL_TIMESTAMP_FORMAT:-%H:%M:%S}"; # see man strftime

PUNCTUAL_CURRENT_DIR_RELATIVE_PATH="${PUNCTUAL_CURRENT_DIR_RELATIVE_PATH:-false}";
PUNCTUAL_CURRENT_DIR_RELATIVE_DEPTH="${PUNCTUAL_CURRENT_DIR_RELATIVE_DEPTH:-3}";
...
punctualCurrentDir () {
    ...
    # Determine path format based on configuration
    local path_format
    if [[ ${PUNCTUAL_CURRENT_DIR_RELATIVE_PATH} = true ]]; then
        # Use relative path with configurable depth
        path_format="%${PUNCTUAL_CURRENT_DIR_RELATIVE_DEPTH}~"
    else
        # Use full path
        path_format='%~'
    fi;

    punctualDecorate "${path_format}" "${PUNCTUAL_CURRENT_DIR_COLOUR}" "${PUNCTUAL_CURRENT_DIR_BOLD}";
}
...
punctualBuildTheme () {
    ...
    if [[ ${PUNCTUAL_SHOW_INDENT} = true ]]; then
        echo -n '  ';
    fi;
