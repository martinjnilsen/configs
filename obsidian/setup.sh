...
PLUGINS_RAW_URL="https://raw.githubusercontent.com/martinjnilsen/configs/main/obsidian/.obsidian/community-plugins.json"
FOLDERS_RAW_URL="https://raw.githubusercontent.com/martinjnilsen/configs/main/obsidian/vault-folders.json"
...
# cmd "Quitting Obsidian before applying config..."
# if $DRY_RUN; then
#   dryrun "Would quit Obsidian so runtime configs can be safely overwritten"
# else
#   debug "Sending quit signal to Obsidian..."
#   osascript -e 'tell application "Obsidian" to quit' 2>/dev/null || true
#   debug "Waiting for Obsidian to exit..."
#   while pgrep -x "Obsidian" > /dev/null 2>&1; do
#     sleep 0.5
#   done
#   debug "Obsidian has exited."
# fi
...
if $DRY_RUN; then
  dryrun "Would clone $CONFIG_GIT_URL (sparse: obsidian/) and copy obsidian/.obsidian/ → .obsidian/"
else
  if [ -d "$TMP_DIR" ]; then
  ...
  cp -r obsidian/.obsidian/. ../.obsidian/
  cd ..
  rm -rf "$TMP_DIR"
...
# ── Remove default Obsidian welcome file ──────────────────────────────────────

cmd "Removing Welcome.md if present..."
if $DRY_RUN; then
  dryrun "Would remove Welcome.md from vault root if it exists"
else
  if [ -f "Welcome.md" ]; then
    debug "Removing Welcome.md..."
    rm -f "Welcome.md"
    info "Removed Welcome.md"
  else
    debug "Welcome.md not found, skipping."
  fi
fi

# ── Reload ────────────────────────────────────────────────────────────────────
# Alternative: comment this block and uncomment "Open Obsidian" below.
...
# ── Open Obsidian (alternative to Reload) ─────────────────────────────────────
# Use this block instead of Reload if Obsidian was quit before config was applied.
# Requires the Quit Obsidian block above to be uncommented as well.

# cmd "Opening Obsidian..."
# if $DRY_RUN; then
#   dryrun "Would open Obsidian: open -a Obsidian"
#   echo ""
#   echo "[dry-run] Preview complete. Run without --dry-run to apply."
# else
#   debug "Running: open -a Obsidian"
#   open -a Obsidian 2>/dev/null || warning "Could not open Obsidian automatically. Please open it manually."
#   debug "Open command sent."
#   echo "[info] Done. Your vault is configured."
# fi